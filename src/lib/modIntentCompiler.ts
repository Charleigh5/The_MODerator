import { resolveCapabilities, type Universe, type CapabilityMatch } from "./capabilityResolver.js";
...
function splitEffects(prompt:string):string[]{return prompt.split(/(?:,|;|\band\b|\bplus\b)/i).map(s=>s.trim()).filter(s=>s.length>2);}
...
function inferMissing(prompt:string,effects:ModEffect[]):string[]{
 ...
 if(effects.some(e=>e.matches.some(m=>/SAVE_FRTK|ROSTER_CONTAINER/.test(m.capability.surface)))&&!/save|dynasty file|roster file/.test(q))miss.push("source save/roster file");
 ...
 if(effects.some(e=>e.matches.some(m=>m.capability.id.startsWith("TB-")))&&!/team builder/.test(q))miss.push("Team Builder vs existing-team Frostbite route");
 return[...new Set(miss)];
}
function replies(missing:string[]):string[]{const r:string[]=[];for(const x of missing){if(x.includes("save"))r.push("Attach copied dynasty save","Attach roster file");else if(x.includes("player"))r.push("Select player by team + position + name/#");else if(x.includes("rating"))r.push("Set exact values","Use realistic position preset");else if(x.includes("Team Builder"))r.push("Team Builder project","Existing-team .fbmod","Show both routes");}return[...new Set(r)].slice(0,6);}
export function compileModIntent(prompt:string,build?:string):ModIntentSpec{
 ...
 const effects=parts.map(raw=>{const matches=resolveCapabilities(raw,universe,6);const x=disposition(matches,universe);return{raw,matches,disposition:x.d,reason:x.r};});
 if(effects.every(e=>e.matches.length===0)){const matches=resolveCapabilities(prompt,universe,8);const x=disposition(matches,universe);effects.splice(0,effects.length,{raw:prompt,matches,disposition:x.d,reason:x.r});}
