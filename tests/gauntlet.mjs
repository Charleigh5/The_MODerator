...
for (const [name, prompt, expected, ready] of cases) {
  const s = compileModIntent(prompt, "test-build");
  const got = ids(s);
  for (const id of expected) assert(got.has(id), `${name}: missing ${id}`);
  if (ready) assert.equal(s.readiness, ready, `${name}: readiness`);
  const receipt = createProofReceipt(s);
  assert.equal(receipt.runtimeClaim, "NOT_RUN");
  assert(buildRecipes(s).length > 0);
  const bundle = createEvidenceBundle(s);
  assert(bundle.id.startsWith("moderator.plan."));
  assert(bundle.files.every((f) => f.lang === "json"));
  assert(bundle.files.some((f) => f.path.endsWith("proof-receipt.json") && f.content.includes("NOT_RUN")));
  console.log(`PASS ${name}: ${s.readiness} -> ${[...got].slice(0, 6).join(",")}`);
}
...
const pilePrimary = pile.effects.flatMap((e) => e.matches.slice(0, 1).map((m) => m.capability.id));
assert(pilePrimary.includes("FTC-ANIM-TARGET") || pilePrimary.includes("RUNTIME-INSTRUMENT"), "pile-leap primary route must be mechanics/animation, not a scalar rating edit");
assert.notEqual(pile.effects[0]?.matches[0]?.capability.id, "PLYR-RATINGS-51", "word 'tackle' must not promote ratings over pile-leap mechanics");
console.log("PASS mechanics-routing: pile-leap intent prioritizes animation/runtime surfaces over ratings");
...
const m = compileModIntent("Madden 27: in my franchise save set my QB speed to 95");
assert.equal(m.readiness, "BLOCKED");
const blockedRecipes = buildRecipes(m);
assert(blockedRecipes.length > 0, "blocked intent may still expose diagnostic recipes");
assert(blockedRecipes.every((r) => r.status === "BLOCKED"), "blocked intent must never emit an executable recipe status");
console.log("PASS madden-parity: save-field request blocks and every generated diagnostic recipe remains BLOCKED");

const needsInputRecipes = buildRecipes(tb);
assert(needsInputRecipes.every((r) => r.status === "NEEDS_INPUT"), "missing material inputs must not be labeled executable");
console.log("PASS recipe-readiness: missing-input plans remain NEEDS_INPUT");

const ont = buildGameOntology();
assert.equal(ont.capabilityCount, 58);
assert(ont.domainCount >= 14);
console.log(`PASS ontology: ${ont.capabilityCount} capabilities / ${ont.domainCount} domains`);
assert.equal(MADDEN27_PROFILE.assetLoader, CFB27_PROFILE.assetLoader);
assert.equal(MADDEN27_PROFILE.canImportMeshes, false);
assert.equal(CFB27_PROFILE.canImportMeshes, true);
console.log("PASS profile parity: shared loader + explicit mesh/launch differences retained");
assert.equal(TEAM_BUILDER_SOURCE_BOUNDARY.status, "ZIP_PACKAGED_SOURCE_NOT_LINE_EXTRACTED");
console.log("PASS Team Builder truth boundary: ZIP internals not fabricated");
